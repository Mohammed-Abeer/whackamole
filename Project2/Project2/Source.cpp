...
for(i=0;i<4;i++)
{
	for(j=0;j<4;j++)
	{
		iShowBMP(hole_Row_dx[i],hole_Col_dy[j],holes[hole[k]]);    // holes and their respective images are screened by using structure.
		k++;
	}
}
if(pause==0)
	...
	if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
{
	if(gameState==1)
{
	left_mouse=game(mx,my,hole_Row_dx, hole_Col_dy, hole);
...
void change()
{
	...
	for(k=0;k<16;k++)
	{
		hole[k]=rand()%4; //moles randomly pops up from hole
	}
	}
}
void time()
